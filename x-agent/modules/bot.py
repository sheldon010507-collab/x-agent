...
"""
    ...
    async def cmd_score(self, update: Update, context: CallbackContext):
        ...
        if not context.args:
            await update.message.reply_text(
                "请提供话题\n\n用法: `/score <话题>`", parse_mode="Markdown"
            )
            return
...
"""
        ...
        try:
            if self.generator:
                result = await self.generator.generate(
                    content_type=content_type, topic=current_topic, niche=current_niche
                )
    ...
    async def cmd_queue(self, update: Update, context: CallbackContext):
        """处理 /queue 命令"""
        await update.message.reply_text(
            "📋 **草稿队列**\n\n暂无草稿\n\n使用 /create 创建内容", parse_mode="Markdown"
        )
...
def create_bot(
    token: str, db=None, llm_router=None, generator=None, openclaw_bridge=None
) -> XAgentBot:
    """创建 Bot 实例"""
