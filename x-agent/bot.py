    ...
    async def cmd_set_niche(self, update: Update, context: CallbackContext) -> None:
        ...
        for niche_id, niche_name in niche_options:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"{niche_name} ({niche_id})", callback_data=f"set_niche_{niche_id}"
                    )
                ]
            )
        reply_markup = InlineKeyboardMarkup(keyboard)
...
""".strip()
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(
            report_text, parse_mode="Markdown", reply_markup=reply_markup
        )
...
""".strip()
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text(
            strategy_text, parse_mode="Markdown", reply_markup=reply_markup
        )
...
""".strip()
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    f"💬 评论 [{'开' if auto.auto_comment_enabled else '关'}]",
                    callback_data="settings_comment_toggle",
                ),
                InlineKeyboardButton(
                    f"上限 {auto.auto_comment_limit}", callback_data="settings_comment_limit"
                ),
            ],
            [
                InlineKeyboardButton(
                    f"👍 点赞 [{'开' if auto.auto_like_enabled else '关'}]",
                    callback_data="settings_like_toggle",
                ),
                InlineKeyboardButton(
                    f"上限 {auto.auto_like_limit}", callback_data="settings_like_limit"
                ),
            ],
            [
                InlineKeyboardButton(
                    f"🔁 转发 [{'开' if auto.auto_rt_enabled else '关'}]",
                    callback_data="settings_rt_toggle",
                ),
                InlineKeyboardButton(
                    f"上限 {auto.auto_rt_limit}", callback_data="settings_rt_limit"
                ),
            ],
            [
                InlineKeyboardButton(
                    f"🤖 自动发帖 [{'开' if auto.auto_post_enabled else '关'}]",
                    callback_data="settings_post_toggle",
                ),
            ],
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
