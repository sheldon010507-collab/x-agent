...
class TestV0FinalResearch:
    ...
    def test_research_topic_with_fallback(self):
        ...
        # 测试正常研究流程
        result = researcher.research_topic(niche="ai_tools", days=7, sources="x,reddit,youtube")
...
class TestNativeResearch:
    ...
    def test_research_topic_structure(self):
        """测试研究返回结构"""
        researcher = Researcher(config=None)
        result = researcher.research_topic(niche="ai_tools", days=7, sources="reddit,hackernews")
    ...
    def test_risk_score_calculation(self):
        ...
        # 测试低风险数据
        low_risk_metrics = {"velocity": 30, "platform_count": 4, "authority": 70}
        low_score = researcher._calculate_risk_score(low_risk_metrics)
        assert low_score < 50, f"低风险数据应该 < 50，实际: {low_score}"

        # 测试高风险数据
        high_risk_metrics = {"velocity": 90, "platform_count": 1, "authority": 30}
    ...
    def test_platform_fetcher_mock(self):
        """测试平台获取器模拟数据"""
        from modules.research import HackerNewsFetcher, RedditFetcher
