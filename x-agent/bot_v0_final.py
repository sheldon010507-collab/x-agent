"""
bot_v0_final.py - X-Agent v0 Final 强制人工审核版
...
class XAgentBotV0Final:
    """
    X-Agent v0 Final Bot - 强制人工审核模式

    核心原则：
    1. 所有内容生成都需经过二次人工确认才能发布
    2. 无自动发布路径，即使 risk_score < 80
    3. 显示 risk_score，帮助用户评估内容质量
    4. 每日 21:00 自动推送复盘报告
    """
    ...
    async def initialize(self) -> None:
        ...
        self.application.add_handler(
            CallbackQueryHandler(
                self.button_callback, pattern=r"^(confirm|auto|regen|skip|publish)_"
            )
        )
    ...
    async def cmd_create(self, update: Update, context: CallbackContext) -> None:
        ...
        research_result = self.researcher.research_topic(
            niche=self.config.get("niche", "general"), days=7
        )
        ...
        generated = self.generator.generate(
            niche=self.config.get("niche", "general"), research=research_result, score=score_result
        )
